...
// IMPORTANT FIX: Original code matches `await addDoc(...)` 
// but wait, activeChannel is used. 
// `await addDoc(collection(db, 'inbox', partnerUsername, 'messages'), { ...payload, receiverUsername: partnerUsername });`
content = content.replace(
...
// Move sector and profession inside min-w-0 and add unread badge
content = content.replace(
  /<span className="text-\[9px\] text-slate-500 bg-white px-1 rounded border border-slate-200 shadow-sm truncate max-w-full">SECTOR \{user\.sector\.toUpperCase\(\)\}<\/span>\n\s*<\/div>\n\s*<p className="text-\[9px\] text-slate-500 truncate mt-0\.5 uppercase tracking-wide" title=\{user\.profession\}>\{user\.profession\} · BOX \{user\.box\}<\/p>\n\s*<\/div>/g,
  `<span className="text-[9px] text-slate-500 bg-white px-1 rounded border border-slate-200 shadow-sm truncate max-w-full">SECTOR {user.sector.toUpperCase()}</span>
                               </div>
                               <p className="text-[9px] text-slate-500 truncate mt-0.5 uppercase tracking-wide" title={user.profession}>{user.profession} · BOX {user.box}</p>
                             </div>
                             {unread && (
                               <div className="flex flex-col items-end shrink-0 ml-1">
                                 ...
                                 <span className="text-[8px] text-slate-400 mt-0.5 whitespace-nowrap">{formatTime(unread.lastTime)}</span>
                               </div>
                             )}
                           </div>`
);
  ...
  `<p className={$1'text-slate-700'}\}>Noticias</p>
                      {channelUnread['noticias']?.count > 0 && (
                        <div className="absolute right-2 top-2 flex flex-col items-end">
                           ...
                           <span className="text-[8px] text-slate-500 mt-0.5 whitespace-nowrap">{formatTime(channelUnread['noticias'].lastTime)}</span>
                        </div>
                      )}`
);
  ...
  `<p className={$1'text-slate-700'}\}>Grupo General</p>
                      {channelUnread['general']?.count > 0 && (
                        <div className="absolute right-2 top-2 flex flex-col items-end">
                           ...
                           <span className="text-[8px] text-slate-500 mt-0.5 whitespace-nowrap">{formatTime(channelUnread['general'].lastTime)}</span>
                        </div>
                      )}`
);
