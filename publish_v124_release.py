...
def create_or_get_release():
    ...
    payload = {
        ...
        "body": f"Release {VERSION} - Python PyWebView Executable & APPDATA Permanent Storage & Top Bar Zoom Controls",
        "draft": False,
        "prerelease": False,
        "make_latest": "true"
    }
...
def delete_existing_asset(release_id, asset_name):
    ...
    try:
        ...
        print(f"Notice during asset deletion: {e}")
...
def main():
    ...
    sha512_hash = hashlib.sha512(exe_bytes).hexdigest()

    yml_content = f"""version: {VERSION}
files:
  - url: Portal-Medico-Setup-1.2.4.exe
    sha512: {sha512_hash}
    size: {len(exe_bytes)}
  - url: portal-medico-setup-1.2.4.exe
    sha512: {sha512_hash}
  ...
  - url: PORTAL MÉDICO.exe
    sha512: {sha512_hash}
    size: {len(exe_bytes)}
path: Portal-Medico-Setup-1.2.4.exe
sha512: {sha512_hash}
releaseDate: '2026-08-17T12:00:00.000Z'
""".encode('utf-8')

    release_id, upload_url = create_or_get_release()

    exe_names_to_upload = [
        "Portal-Medico-Setup-1.2.4.exe",
        "portal-medico-setup-1.2.4.exe",
        "PortalMedico.exe",
        "PORTAL MÉDICO.exe",
        "PORTAL MÉDICO Setup 1.2.4.exe"
    ]
    ...
    print("\nRELEASE V1.2.4 PYTHON PYWEBVIEW EXECUTABLE SUCCESSFULLY PUBLISHED TO GITHUB!")
