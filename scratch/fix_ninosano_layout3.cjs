...
files.forEach(file => {
  ...
  content = content.replace(/<section key=\{section\.title\} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">/g, `<section key={section.title} className="bg-[#F8FAFC] rounded-xl shadow-sm border border-slate-200 p-4 sm:p-5 flex flex-col gap-2">`);
  ...
  if (file.includes("6Anos")) {
      // 6Anos has <div className="w-full relative"> inside <> </>. 
      // We will remove the starting <> and ending </>
