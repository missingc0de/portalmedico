...
TAG = "v1.2.2"
VERSION = "1.2.2"
SHA512_HASH = "QC5D2C8vYnUnI5+Pw/9jbl+wezzBTCvGCSNkOHyk+KAy43WGzHpjvpM+KfjG4rbzKQBViAkboFkYre6YMjqsfw=="
...
def create_or_get_release():
    ...
    payload = {
        ...
        "body": f"Release {VERSION} - Python PyWebView Executable with Embedded Icon & Desktop Shortcut Overwrite",
        "draft": False,
        "prerelease": False,
        "make_latest": "true"
    }
...
files:
  - url: Portal-Medico-Setup-1.2.2.exe
    sha512: {SHA512_HASH}
    size: {len(exe_bytes)}
  - url: portal-medico-setup-1.2.2.exe
    sha512: {SHA512_HASH}
  ...
  - url: PORTAL MÉDICO.exe
    sha512: {SHA512_HASH}
    size: {len(exe_bytes)}
path: Portal-Medico-Setup-1.2.2.exe
sha512: {SHA512_HASH}
releaseDate: '2026-08-16T23:38:00.000Z'
""".encode('utf-8')

    release_id, upload_url = create_or_get_release()

    exe_names_to_upload = [
        "Portal-Medico-Setup-1.2.2.exe",
        "portal-medico-setup-1.2.2.exe",
        "PortalMedico.exe",
        "PORTAL MÉDICO.exe",
        "PORTAL MÉDICO Setup 1.2.2.exe"
    ]
    ...
    print("\nRELEASE V1.2.2 WITH EMBEDDED LOGO AND SHORTCUT OVERWRITE SUCCESSFULLY PUBLISHED!")
