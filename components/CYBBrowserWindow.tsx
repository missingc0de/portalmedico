...
interface CYBBrowserWindowProps {
  isOpen: boolean;
  onClose: () => void;
  onOpen?: () => void;
}

const INITIAL_URL = 'https://portal.cybserviciosmedicos.cl/PortalLoginConvenio';
const DEFAULT_WIDTH = 680;
const DEFAULT_HEIGHT = 520;
const MIN_WIDTH = 400;
const MIN_HEIGHT = 300;

export const CYBBrowserWindow: React.FC<CYBBrowserWindowProps> = ({ isOpen, onClose, onOpen }) => {
  const [tabs, setTabs] = useState<Tab[]>([
    { id: 'main-tab', url: INITIAL_URL, title: 'CYB Servicios Médicos', loading: true, urlHistory: [INITIAL_URL] }
  ]);
  ...
  const getTabTitle = (url: string) => {
    if (url.includes('convenios')) return 'Convenios';
    if (url.includes('informes')) return 'Informes';
    if (url.includes('pacientes')) return 'Pacientes';
    return 'Módulo CYB';
  };
  ...
  useEffect(() => {
    ...
    (window as any).__navigateCybFrame = handleNavigate;
    (window as any).__openCybTab = handleNavigate;
    navigateRef.current = handleNavigate;

    return () => {
      delete (window as any).__navigateCybFrame;
      delete (window as any).__openCybTab;
    };
  }, [popoutCount, onOpen]);
  ...
  useEffect(() => {
    const electronAPI = (window as any).electronAPI;
    if (electronAPI && electronAPI.onWebviewNewWindowCyb) {
      const unsubscribe = electronAPI.onWebviewNewWindowCyb((data: { url: string }) => {
        ...
        {!isElectron() ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 bg-slate-50 text-center">
            ...
            <p className="text-xs text-slate-500 max-w-sm mb-4 leading-relaxed">
              Estás usando un navegador web externo. Por seguridad, el servidor de <b>CYB Servicios Médicos</b> prohíbe incrustarse en otras páginas web (bloqueo X-Frame/CSP).
            </p>
